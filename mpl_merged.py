...
class MPLClient:
    ...
    def __init__(self, auto_login: bool = True, mock_mode: bool = False):
        ...
        Args:
            ...
            mock_mode: If True, run in mock mode for testing without real MEGA credentials
        """
        self.auto_login = auto_login
        self.mock_mode = mock_mode
        self._event_callbacks = {}
        
        # MEGAcmd compatibility: Working directory state  
        self._current_directory = "/"
        self._local_directory = os.getcwd()
    ...
    # Mock mode overrides for testing
    def login(self, email: str, password: str) -> bool:
        """Login to MEGA account (with mock mode support)."""
        if getattr(self, 'mock_mode', False):
            self._mock_logged_in = True
            self._mock_user = email
            self.logger.info(f"Mock login successful for {email}")
            return True
        else:
            try:
    ...
    def logout(self) -> bool:
        """Logout from MEGA account (with mock mode support)."""
        if getattr(self, 'mock_mode', False):
            self._mock_logged_in = False
            self._mock_user = None
            self.logger.info("Mock logout successful")
            return True
        else:
            try:
                return logout_user()
    ...
    def is_logged_in(self) -> bool:
        """Check if user is logged in (with mock mode support)."""
        if getattr(self, 'mock_mode', False):
            return getattr(self, '_mock_logged_in', False)
        else:
            try:
    ...
    def get_current_user(self) -> Optional[str]:
        """Get current user email (with mock mode support)."""
        if getattr(self, 'mock_mode', False):
            return getattr(self, '_mock_user', None)
        else:
            try:
    ...
    def get_user_info(self) -> Optional[dict]:
        """Get current user information (with mock mode support)."""
        if getattr(self, 'mock_mode', False):
            if getattr(self, '_mock_logged_in', False):
                return {
                    'email': getattr(self, '_mock_user', 'mock@example.com'),
                    'name': 'Mock User',
                    'plan': 'Free',
                    'quota': {'used': 0, 'total': 15000000000}
                }
            return None
        else:
            try:
    ...
    def login(self, email: str, password: str, save_session: bool = True) -> bool:
        """
        Log in to Mega (with mock mode support).
    ...
    def get_user_quota(self) -> Dict[str, int]:
        """Get user storage quota information (with mock mode support)."""
        if getattr(self, 'mock_mode', False):
            if getattr(self, '_mock_logged_in', False):
                return {'used': 0, 'total': 15000000000, 'available': 15000000000}
            return {}
...
def add_authentication_methods(client_class):
    ...
    def login_method(self, email: str, password: str, save_session: bool = True) -> bool:
        """
        Log in to Mega with email and password (with mock mode support).
